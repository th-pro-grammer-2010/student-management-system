...
def Database():
    ...
    conn = sqlite3.connect("studentccs_management.db")
    cursor = conn.cursor()
    #creating STUD_REGISTRATION table
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS STUD_REGISTRATION (STU_SN INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, STU_NAME TEXT, STU_CONTACT TEXT, STU_EMAIL TEXT, STU_ROLLNO TEXT, STU_AN TEXT)")
...
def DisplayForm():
    ...
    global tree
    global SEARCH
    global name,contact,email,rollno,an
    SEARCH = StringVar()
    name = StringVar()
    contact = StringVar()
    email = StringVar()
    ...
    Label(LFrom, text="admission no.", font=("Arial", 12)).pack(side=TOP)
    Entry(LFrom, font=("Arial", 10, "bold"),textvariable=an).pack(side=TOP, padx=10, fill=X)
    Button(LFrom,text="Submit",font=("Arial", 10, "bold"),command=register).pack(side=TOP, padx=10,pady=5, fill=X)
    Label(LFrom, text="CREATED BY TUSHAR HARBOLA", font=("Arial", 12)).pack(side=LEFT)
   ...
   #setting scrollbar
    ...
    tree = ttk.Treeview(MidViewForm,columns=("serial no.", "Name", "Contact", "Email","Rollno","admission no."),
    ...
    tree.heading('admission no.', text="admission no.", anchor=W)
...
def register():
    ...
    if name1=='' or con1==''or email1=='' or rol1==''or an1=='':
        tkMessageBox.showinfo("Warning","fill the empty field!!!")
    else:
        #execute query
        conn.execute('INSERT INTO STUD_REGISTRATION (STU_NAME,STU_CONTACT,STU_EMAIL,STU_ROLLNO,STU_AN) \
              VALUES (?,?,?,?,?)',(name1,con1,email1,rol1,an1));
        conn.commit()
        tkMessageBox.showinfo("Message"," Information Stored successfully")
        #refresh table data
        DisplayData()
        conn.close()
...
def Delete():
    ...
    if not tree.selection():
        ...
        if result == 'yes':
            ...
            tree.delete(curItem)
            cursor=conn.execute("DELETE FROM STUD_REGISTRATION WHERE STU_NAME = %d" % selecteditem[0])
            conn.commit()
            cursor.close()
            conn.close()
