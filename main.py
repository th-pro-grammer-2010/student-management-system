...
def Database():
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS STUD_REGISTRATION (STU_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, STU_NAME TEXT, STU_CONTACT TEXT, STU_EMAIL TEXT, STU_ROLLNO TEXT, STU_CLASS TEXT)")
...
def DisplayForm():
    ...
    global tree
    global SEARCH
    global name,contact,email,rollno,clas
    SEARCH = StringVar()
    name = StringVar()
    contact = StringVar()
    email = StringVar()
    ...
    Label(LFrom, text="class ", font=("Arial", 12)).pack(side=TOP)
    Entry(LFrom, font=("Arial", 10, "bold"),textvariable=clas).pack(side=TOP, padx=10, fill=X)
   ...
   #setting scrollbar
    ...
    tree = ttk.Treeview(MidViewForm,columns=("Student Id", "Name", "Contact", "Email","Rollno","Class"),
...
def register():
    ...
    if name1=='' or con1==''or email1=='' or rol1==''or clas1=='':
        tkMessageBox.showinfo("Warning","fill the empty field!!!")
    else:
        #execute query
        conn.execute('INSERT INTO STUD_REGISTRATION (STU_NAME,STU_CONTACT,STU_EMAIL,STU_ROLLNO,STU_CLASS) \
              VALUES (?,?,?,?,?)',(name1,con1,email1,rol1,clas1));
        conn.commit()
